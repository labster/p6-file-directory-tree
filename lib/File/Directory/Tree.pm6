...
multi sub mktree (IO(Cool:D) $path is copy, Int :$mask = 0o777 ) is export {
	return True if $path ~~ :d;
	my @makedirs;
	while $path !~~ :e {
		@makedirs.push($path);
		$path.=parent;
...
multi sub empty-directory (IO(Cool:D) $path) is export {
...
multi sub rmtree (IO(Cool:D) $path) is export {
